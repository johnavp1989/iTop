...
class _IPv6Subnet extends IPSubnet
{
	...
	function DoCheckCIDRAligned()
	{
		...
		$oIp = $this->Get('ip');
		$oLastIp = $this->Get('lastip');	 			
		$oMask = new ormIPv6('FFFF:FFFF:FFFF:FFFF:0:0:0:0');
	...
	function DoCheckOperation($sOperation)
	{
		...
		{
			case 'findspace':
				// There should be always room in /64 !!
				return ('');
			break;
...
EOF
	...
	function DisplayBareRelations(WebPage $oP, $bEditMode = false)
	{
		...
		if ($this->IsNew())
		{
			...
			if ($sOrgId != null)
			{
				...
				$aParameter = array ('reserve_subnet_IPs', null);
		...
		else
		{
			...
			$oIpRegisteredSearch = DBObjectSearch::FromOQL("SELECT IPv6Address AS i WHERE :ip <= i.ip AND i.ip <= :lastip AND i.org_id = $sOrgId",  array('ip' => $sIp, 'lastip' => $sLastIp));
			$oIpRegisteredSet = new CMDBObjectSet($oIpRegisteredSearch);
	...
	public function ComputeValues()
	{
		...
		// Set Last IP
		$oLastIpFromMask = new ormIPv6('::ffff:ffff:ffff:ffff');
